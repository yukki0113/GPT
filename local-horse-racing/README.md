...
- Google Drive `/GPT/local-horse-racing/10_canonical`: rawから再生成する加工後・正規化データ
- Google Drive `/GPT/local-horse-racing/20_audit`: 取得日時、SHA-256、サイズ、ZIP内ファイル一覧など
...
- local-horse-racing固有の列、キー、partition、validation条件はプロジェクト側のconfig / schemaに置き、共通toolのPython実装をコピーしない。
- 共通toolの仕様・test方法は `tools/data-storage/README.md` を正本とする。

Phase 0ではcanonical変換自体はまだ標準フローに含めません。次Phaseで導入する場合は、まず `tools/data-storage/` をimport / CLI利用する方式を第一候補とします。
