...
public sealed class JsonGameSettingCatalogRepository(string catalogPath) : IGameSettingCatalogRepository
{
    ...
    private static GameSettingDefinition Convert(CatalogDto dto)
    {
        ...
        return new GameSettingDefinition { Id = dto.Id ?? string.Empty, DisplayNameJa = dto.DisplayNameJa ?? string.Empty, DisplayNameEn = dto.DisplayNameEn ?? string.Empty, Category = dto.Category ?? string.Empty, UiCategory = dto.UiCategory ?? string.Empty, UiSubCategory = dto.UiSubCategory ?? string.Empty, DescriptionJa = dto.DescriptionJa ?? string.Empty, FileKind = fileKind, Section = dto.Section ?? string.Empty, Key = dto.Key ?? string.Empty, ValueType = ParseValueType(dto.ValueType), DefaultValue = dto.DefaultValue?.ToString(), Minimum = dto.Minimum?.ToString(), Maximum = dto.Maximum?.ToString(), EnumValues = dto.EnumValues ?? [], SupportStatus = ParseSupportStatus(dto.SupportStatus), Deprecated = dto.Deprecated, RestartRequired = dto.RestartRequired, Sources = dto.Source ?? [], Notes = dto.Notes ?? string.Empty };
    }
    ...
    private sealed class CatalogDto { public string? Id { get; set; } public string? DisplayNameJa { get; set; } public string? DisplayNameEn { get; set; } public string? Category { get; set; } public string? UiCategory { get; set; } public string? UiSubCategory { get; set; } public string? DescriptionJa { get; set; } public string? File { get; set; } public string? Section { get; set; } public string? Key { get; set; } public string? ValueType { get; set; } public JsonElement? DefaultValue { get; set; } public JsonElement? Minimum { get; set; } public JsonElement? Maximum { get; set; } public List<string>? EnumValues { get; set; } public string? SupportStatus { get; set; } public bool Deprecated { get; set; } public bool RestartRequired { get; set; } public List<string>? Source { get; set; } public string? Notes { get; set; } }
}
