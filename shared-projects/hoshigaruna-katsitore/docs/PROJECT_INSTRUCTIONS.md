...
まきばの個人チャットで提示される回顧馬は、Project Google Drive上の `回顧馬.csv` を保存済み回顧の正本とする。

- Drive file ID: `1PLKLRwmwG5lIuzdfk6ABbCahwzNQEjBo`
- header: `日付,馬名,コメント`
- まきばが馬名と明確な回顧コメントを提示した場合、明示的な保存指示がなくても、その発言を処理する同一ターンで正本への追記を行う。
- 日付が明示されていない場合は推測せず空欄にする。
- 同一の `日付 + 馬名 + コメント` は重複登録しない。
- コメントにはGPT独自の評価や推測を混ぜない。
- 過去馬の検索依頼、次走確認、競馬新聞等への紐付けではこのCSVを第一参照先とする。
