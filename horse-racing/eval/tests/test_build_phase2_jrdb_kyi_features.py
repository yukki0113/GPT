...
class BuildPhase2JrdbKyiFeaturesTest(unittest.TestCase):
    def test_pre_race_features_are_projected_from_common_parser(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(row["stable_index"], 20.0)
            self.assertEqual(row["stable_evaluation_code"], "4")
            self.assertEqual(row["stable_evaluation_label"], "弱気")
            ...
            self.assertEqual(row["forecast_pace_code"], "H")
            self.assertEqual(row["forecast_pace_label"], "ハイ")
            self.assertEqual(row["expected_front_index"], 70.0)
            self.assertEqual(row["expected_pace_index"], 62.5)
            self.assertEqual(row["expected_late_index"], 58.0)
            self.assertEqual(row["expected_position_index"], 55.5)
            self.assertEqual(row["expected_front_rank"], 1)
            self.assertEqual(row["expected_pace_rank"], 2)
            self.assertEqual(row["expected_late_rank"], 3)
            self.assertEqual(row["expected_position_rank"], 4)
            self.assertEqual(row["start_index"], 8.5)
            self.assertEqual(row["late_break_rate"], 12.5)
