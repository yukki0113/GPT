...
PACKET_VERSION = "RaceNote-Semantic-Pairwise-Research-v0.3"
...
def _condition_profile(horse: Mapping[str, object]) -> dict[str, object]:
    """Keep JRDB condition signals as corroboration context only."""
    interpretation = _mapping(horse.get("prediction_interpretation"))
    signal = _mapping(interpretation.get("jrdb_condition_signal"))
    return {
        "state": _text(signal.get("state")).upper(),
        "jrdb_improvement": signal.get("jrdb_improvement"),
        "jrdb_improvement_relation": _text(
            signal.get("jrdb_improvement_relation")
        ).upper(),
        "jrdb_training_arrow": signal.get("jrdb_training_arrow"),
        "jrdb_training_arrow_relation": _text(
            signal.get("jrdb_training_arrow_relation")
        ).upper(),
        "policy": dict(_mapping(signal.get("policy"))),
    }
...
def _profile(horse: Mapping[str, object]) -> dict[str, object]:
    return {
        ...
        "jrdb_condition_signal": _condition_profile(horse),
...
def _comparison_prompts(
    ...
    return [
        ...
        (
            "JRDB_CONDITION: 上昇度/調教矢印は単独順位スコアにせず、"
            "Trend/Review/Ability判断の補強または反証としてのみ読む。"
        ),
        (
...
def build_packet(
    ...
    candidate_nos = order[: min(6, len(order))]
