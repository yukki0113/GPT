...
SURFACE_TO_CODE={v:k for k,v in TRACK_TYPE.items()}
...
def enrich(base,a,m,older_limit,years):
    d=base["race"]["date"]; vn=base["race"]["venue"]; v=VENUE_TO_CODE[vn]; rn=int(base["race"]["race_no"]); track=SURFACE_TO_CODE.get(base["race"]["surface"],base["race"]["surface"]); dist=int(base["race"]["distance_m"])
