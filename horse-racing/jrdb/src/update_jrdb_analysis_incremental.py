#!/usr/bin/env python3
"""Incrementally replace completed JRDB dates in Analysis Lite v1.3.
...
FACT_COLUMNS = (
    ...
    "race_condition_code", "track_condition_code", "grade_code", "win5_leg_no",
    ...
    "finish", "abnormal_code", "final_win_odds", "final_win_popularity", "win_payout",
    "place_payout", "prev_result_key_1", "prev_race_key_1",
)


def now() -> str:
    """Return a second-resolution local timestamp for ingest metadata."""
    return dt.datetime.now().isoformat(timespec="seconds")


def parse_date(value: str) -> dt.date:
    """Parse a supported compact or separated date string."""
    return dt.datetime.strptime(value.replace("-", "").replace("/", ""), "%Y%m%d").date()


def infer_date_from_name(path: Path, prefix: str) -> dt.date:
    """Infer the target date from a PACI/SED daily ZIP filename."""
...
def sha256_file(path: Path) -> str:
    """Return SHA-256 for one Raw archive."""
...
def daily_zip(raw_root: Path, kind: str, date: dt.date) -> Path:
    """Resolve one daily kind ZIP from the standard Raw layout."""
    return raw_root / kind / f"{kind}{date:%y%m%d}.zip"


def read_member(path: Path, kind: str, date: dt.date) -> list[bytes]:
    """Read one validated fixed-width member from a daily archive."""
    if not path.exists():
...
def _parse_lines(
    ...
    """Join one completed race date into Analysis fact rows."""
    ...
    for key, entry in entries.items():
        ...
        rows.append((
            ...
            race["track_condition_code"], race["grade_code"], race["win5_leg_no"],
            key[0], key[1], entry["frame_no"], entry["horse_id"], entry["horse_name"],
            profile.get("sex_code"), age, profile.get("sire_name"),
            profile.get("broodmare_sire_name"), profile.get("sire_line_code"),
            profile.get("broodmare_sire_line_code"), entry["jockey_name"],
            entry["running_style"], entry["distance_aptitude"], entry["uptrend"],
            training.get(key), result["finish"], result["abnormal_code"],
            result["final_win_odds"], result["final_win_popularity"],
            result["win_payout"], result["place_payout"], entry["prev_result_key_1"],
            entry["prev_race_key_1"],
        ))

    if not rows:
        raise ValueError(f"{date}: no joinable completed rows")
    return rows, {
        "race_count": len({row[10] for row in rows}),
        "row_count": len(rows),
...
def parse_paci_sed(
    paci: Path,
    sed: Path,
) -> tuple[dt.date, list[tuple], dict[str, object]]:
    """Parse one completed date from a PACI + SED pair."""
    if not paci.exists():
...
def ensure_v13(conn: sqlite3.Connection) -> None:
    """Reject databases that do not implement the v1.3 Analysis contract."""
    columns = {
        row[1]
        for row in conn.execute("PRAGMA table_info(fact_entry_result_lite)")
    }
    missing = {"prev_result_key_1", "prev_race_key_1", "win5_leg_no"} - columns
    if missing:
        raise RuntimeError(f"Analysis DB is not v1.3; missing columns: {sorted(missing)}")
    tables = {
        row[0]
        for row in conn.execute("SELECT name FROM sqlite_master WHERE type='table'")
    }
    if "meta_analysis_ingest_batch" not in tables:
        raise RuntimeError("Analysis DB is not v1.3; missing meta_analysis_ingest_batch")
...
def update_rows(
    ...
    """Atomically replace one race date and record ingest metadata."""
    started = now()
    batch_id = conn.execute(
        ...
        (
            date.isoformat(), VERSION, SCHEMA_VERSION, started, "RUNNING",
            json.dumps(meta["source_manifest"], ensure_ascii=False),
            json.dumps(meta["source_sha256s"]), meta["race_count"], meta["row_count"],
        ),
    ).lastrowid
    conn.commit()
    try:
        ...
        conn.execute(
            "DELETE FROM fact_entry_result_lite WHERE race_date=?",
            (date.isoformat(),),
        )
        sql = (
            f"INSERT INTO fact_entry_result_lite({','.join(FACT_COLUMNS)}) "
        ...
        if inserted != len(rows):
            raise RuntimeError(
                f"inserted row mismatch: expected {len(rows)}, got {inserted}"
            )
        conn.execute("COMMIT")
        message = (
...
def update_date(
    ...
    """Parse and replace one date from daily-kind Raw archives."""
...
def main() -> None:
    ...
    try:
        ...
        print(json.dumps(
            {"updates": output, "integrity_check": integrity},
            ensure_ascii=False,
            indent=2,
        ))
    finally:
