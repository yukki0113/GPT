...
def write_sqlite(conn:duckdb.DuckDBPyConnection,out:Path,schema:Path,source:str)->dict:
...
def build(analysis_glob:Path,sqlite_out:Path,parquet_dir:Path,schema:Path)->dict:
 if sqlite_out.exists() or parquet_dir.exists():raise FileExistsError("refusing overwrite")
 c=duckdb.connect(":memory:"); c.execute(f"CREATE VIEW source AS SELECT * FROM read_parquet({q(str(analysis_glob))}, hive_partitioning=true, union_by_name=true)")
 ...
 c.execute("CREATE TABLE dim_race AS SELECT row_number() over(order by race_key)::INTEGER id,race_key, max(nullif(trim(race_name),'')) race_name FROM source GROUP BY race_key") if "race_name" in [x[0] for x in c.execute("DESCRIBE source").fetchall()] else c.execute("CREATE TABLE dim_race AS SELECT row_number() over(order by race_key)::INTEGER id,race_key, CAST(NULL AS VARCHAR) race_name FROM (SELECT DISTINCT race_key FROM source)")
 ...
 return audit
def main():
 p=argparse.ArgumentParser();p.add_argument("--analysis-parquet",type=Path,required=True);p.add_argument("--sqlite-out",type=Path,required=True);p.add_argument("--parquet-dir",type=Path,required=True);p.add_argument("--schema",type=Path,default=Path(__file__).resolve().parents[1]/"schema"/"jrdb_pwa_fact_lite_schema_v0_3.sql");a=p.parse_args();print(build(a.analysis_parquet,a.sqlite_out,a.parquet_dir,a.schema))
