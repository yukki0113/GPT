...
AUTHOR_VERSION = "RaceNote-Semantic-Author-Research-v0.2"
...
def judge_pair(a: Mapping[str, object], b: Mapping[str, object]) -> dict[str, object]:
    ...
    # Trend remains the first lane, but only a content-level directional
    # relation receives that protection.  Coarse state alone is never decisive.
    if trend_rel in {"A", "B"} and review_rel in {trend_rel, "EVEN"}:
        final = trend_rel
        decisive = "DATA_TREND_CONTENT"
    elif trend_rel in {"EVEN", "MIXED"} and review_rel in {"A", "B"}:
        final = review_rel
        decisive = "RACEREVIEW_CONTENT"
    elif trend_rel == review_rel and trend_rel in {"A", "B"}:
        final = trend_rel
        decisive = "MIXED_CONTENT"
    elif trend_rel in {"EVEN", "MIXED"} and review_rel in {"EVEN", "MIXED"}:
        if ability_rel in {"A", "B"}:
            final = ability_rel
        ...
        # Conflicting Trend and RaceReview are intentionally not broken by a
        # lower Ability lane in this research version.
        final = "EVEN"
        decisive = "CROSS_LANE_CONFLICT"
