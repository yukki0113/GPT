...
AUTHOR_VERSION = "RaceNote-Semantic-Author-Research-v0.4"
...
def _condition_relation(
    ...
    """Compare JRDB condition arrows only as corroboration/contradiction."""
    ca = _mapping(a.get("jrdb_condition_signal"))
    cb = _mapping(b.get("jrdb_condition_signal"))
    state_a = _text(ca.get("state")).upper()
    state_b = _text(cb.get("state")).upper()

    positive = {"SUPPORTIVE"}
    negative = {"CONCERN"}
    neutral = {"NEUTRAL", "UNAVAILABLE", "MIXED", ""}

    relation = "EVEN"
    if state_a in positive and state_b in negative | neutral:
        relation = "A"
    elif state_b in positive and state_a in negative | neutral:
        relation = "B"
    elif state_a in negative and state_b in neutral:
        relation = "B"
    elif state_b in negative and state_a in neutral:
        relation = "A"

    return {
        "relation": relation,
        "state_a": state_a or "UNAVAILABLE",
        "state_b": state_b or "UNAVAILABLE",
        "summary": (
            "JRDB improvement/training-arrow signal may corroborate or veto "
            "a forecast-evidence reversal, but may never create one alone."
        ),
    }
...
def judge_pair(a: Mapping[str, object], b: Mapping[str, object]) -> dict[str, object]:
    ...
    condition = _condition_relation(a, b)

    trend_rel = _text(trend.get("relation"))
    review_rel = _text(review.get("relation"))
    ability_rel = _text(ability.get("relation"))
    condition_rel = _text(condition.get("relation"))

    # First derive a relation from forecast evidence only.
    ...
    # JRDB condition signal is deliberately downstream of the forecast
    # evidence relation. It can confirm or veto a proposed reversal, but it
    # cannot manufacture a preference from an unresolved comparison.
    if final in {"A", "B"}:
        opposite = "B" if final == "A" else "A"
        if condition_rel == opposite:
            final = "EVEN"
            decisive = decisive + "_CONDITION_CONTRADICTION"
        elif condition_rel == final:
            decisive = decisive + "_CONDITION_CORROBORATED"
    ...
    return {
        ...
        "racereview": review,
        "ability": ability,
        "jrdb_condition_signal": condition,
    }
