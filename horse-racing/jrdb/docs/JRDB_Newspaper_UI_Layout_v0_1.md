...
### 7.1 Standard past-run cell

通常のdetailed過去走セルは、スマホ上で次の7行相当を標準とする。
...
5. 騎手 / 斤量 / 馬体重（増減）
6. 走破時計 / 1(2)着馬とのタイム差
7. コーナー位置取り / 上がり3F（実タイム順位があれば順位も表示）
```

`time_gap_sec` はJRDB SED/ZEDの `1(2)着タイム差` を投影する。2着以下では勝馬との差、勝馬では2着馬との差を意味し、推測計算で補完しない。

`last3f_rank` は**実際の上がり3Fタイム順位**だけを格納する。JRDBの上がり指数順位等、意味の異なる順位を代用しない。正確な順位を取得できないsourceではnullのまま保持する。

上がり3F順位が1～3位の場合は視認性のため色分けしてよい。初期候補は1位=淡黄、2位=淡青、3位=淡緑とする。順位がnullの場合は色分けしない。

compact 6-8走目は上記項目を無理に埋めず、sourceに存在する値だけを表示する。
