...
2010–2025でDiscovery / Validation / Holdoutを通過したJRDB Edge Registryを、2026以降の完全なForward期間で継続監査する。
...
Full 2010–2025 Registryを実PACIへpre-race適用し、当日SEDを事後settlement専用で使用した初回shadow auditでは、946 runnerすべてをSEDへexact joinできた。

この数値は運用判断の確定値ではなく、Forward evaluator実装・接続確認用のinitial smokeとして扱う。
