...
- Analysis Lite / Stats Mart / RaceNote Archive などの大容量artifactは再生成・世代更新でファイル名やGoogle Drive File IDが変わり得るため、Gitには個別のDrive URL / File IDを固定しない。
...
## RaceNote Archive design
- 過去RaceNoteの大量・反復取得用に `RaceNote Archive` をhistorical base delivery cacheとして追加する設計。詳細は `docs/RaceNote_Archive_Design_v0_1.md`、SQLite schema draftは `schema/racenote_archive_schema_v1_0.sql`。
...
- Archive schema v1.0はRaceNote bundle schema v1.0とは別version軸。初期shardは1暦月1 SQLite、1race=1 zlib-compressed JSON BLOB、lookup keyは `race_date + venue_code + race_no`。
- 過去requestはArchiveを優先し、Archive未整備・validation失敗時は既存safe fallback（2026+ PACI / <=2025 annual Raw reconstruction）を維持する。
- ArchiveはRaw/Coreを置換しない。Raw/Coreはaudit/rebuild source truthのまま、Archiveは高速delivery層だけを担当する。
- 大容量Archive shardはGit外ストレージ。固定命名 `jrdb_racenote_archive_YYYYMM_v1_0.sqlite` と外部manifestで探索し、Drive URL / File IDをGitへ固定しない。
- 初期coverage候補はAnalysis Lite通常利用期間に合わせ2016年以降。実装前後に2025 historical Raw系・2026 PACI系のsemantic equalityと1か月full-build auditを必須とする。
