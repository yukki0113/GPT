...
def _occurrence(*, edge_id: str = "EDGE-A", horse_no: int = 1, place_payout: int = 160) -> dict:
    return {
        "runner_identity": f"01262501:{horse_no:02d}",
...
def test_empty_day_requires_explicit_date_and_is_recorded(tmp_path: Path) -> None:
    ...
    connection = sqlite3.connect(tmp_path / "ledger.sqlite")
    try:
        imported = ledger.import_daily_audit(connection, audit, race_date="2026-09-12")
        report = ledger.summarize(connection)
    finally:
        connection.close()
