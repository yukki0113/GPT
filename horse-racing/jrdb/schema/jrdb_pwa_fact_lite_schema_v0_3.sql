...
CREATE TABLE fact_stats_entry(
  ...
  prev_distance_delta INTEGER,
  prev_class_code INTEGER,
  win5_leg_no INTEGER CHECK(win5_leg_no IS NULL OR win5_leg_no BETWEEN 1 AND 5)
);

CREATE INDEX ix_pwa_fact_course
ON fact_stats_entry(year, month, venue_code, track_type, distance, track_condition_code);

CREATE INDEX ix_pwa_fact_date
ON fact_stats_entry(race_date_int);

CREATE INDEX ix_pwa_fact_race
ON fact_stats_entry(race_id);

CREATE INDEX ix_pwa_fact_win5
ON fact_stats_entry(win5_leg_no)
